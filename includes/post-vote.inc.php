...
if (isset($_GET['topic']) && isset($_GET['post']) && isset($_GET['action']) 
        && isset($_SESSION['userId'])){
    if($_GET['action'] == 'up'){
        /* Select vote from postvotes where voteBy =? and votepost = ? and vote = 1 */
        /* Vote is already given */
        /* vote = 1*/
        /* else insert into postvotes values(vote) =? where voteBy =? and votepost = ?*/
    }elseif($_GET['action'] == 'down'){
        /* Select vote from postvotes where voteBy =? and votepost = ? */
        /* Vote is already given */
        if(given){
            /*update vote = 0 where voteBy =? and votepost = ? */
        }else{
           /* else insert into postvotes values(vote) = 0 where voteBy =? and votepost = ?*/ 
        }
    }
}
